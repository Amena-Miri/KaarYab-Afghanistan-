...
const OpportunityDetailsPage = () => {
  ...
  const [opportunity, setOpportunity] = useState(
    getOpportunityById(params.id as string)
  );
    ...
    if (opp) {
      const related = opportunities
        .filter(
          (item) =>
            item.id !== opp.id &&
            (item.category === opp.category ||
              item.tags.some((tag) => opp.tags.includes(tag)))
        )
        .slice(0, 3);
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div {...fadeUp} transition={{ delay: 0.1 }}>
              <Card className="rounded-3xl border border-border bg-surface p-6">
                ...
                <p className="text-text-secondary leading-7 whitespace-pre-wrap">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <h3 className="flex items-center gap-2 text-xl font-bold text-text-primary">
                      <Sparkles className="w-5 h-5 text-primary" /> Ready to
                      Apply?
                    </h3>
                ...
                <div className="space-y-4">
                  {[
                        ...
                        <>
                          ...
                          {!expired && (
                            <p className="text-sm text-success">
                              {getDaysRemaining(opportunity.deadline)} days
                              remaining
                            </p>
                          )}
                        </>
                    ...
                    },
                    {
                      icon: Eye,
                      label: "Views",
                      value: opportunity.views || 0,
                    },
                    {
                      icon: Heart,
                      label: "Saves",
                      value: opportunity.saves || 0,
                    },
                  ].map((item, idx) => (
